...
- **README 首次接入体验优化**：将“飞书应用准备”前移到文档开头，补充自建应用创建、OAuth 回调地址、最小权限集合、Wiki 侧「添加文档应用」等首次使用必需信息；同时重排已发版 npm 包的 `mcp.json` 接入说明，让用户可以先完成应用配置，再直接复制客户端配置开始使用。
- **示例配置脱敏与一致性增强**：将 `.env.example` 中的飞书应用凭据替换为占位符，避免示例文件误导用户；README 与仓库根目录 `mcp.json` 继续保持面向已发布包的接入口径。
- **OAuth 默认 scope 对齐文档能力**：将默认授权范围补充 `drive:drive`，让 OAuth 授权结果与文档中声明的上传、导入、媒体与云空间能力保持一致，并补充单元测试覆盖。
- **版本号更新**：将版本号从 `2.6.6` 更新为 `2.6.7`，同步更新 `package.json`、`server.json` 与 `README.md` 中的版本标识。
...
### Changed
- - **飞书文档媒体批量下载与接口限制对齐**：将单次批量获取临时下载链接时的 `file_token` 上限从 20 调整为 5，与飞书接口实际限制一致；超出时自动拆分为多次请求。在批量拉取临时链接路径中补充请求上下文，便于排查失败原因。单元测试覆盖「超限时拆分请求」行为。

- **版本号更新**：将版本号从 `2.6.5` 更新为 `2.6.6`，同步更新 `package.json`、`server.json` 与 `README.md` 中的版本标识。
...
- **文档读取与媒体处理增强**：完善飞书文档读取链路，增强多模态内容输出能力，并支持远程媒体资源下载与处理，提升包含图片等富媒体内容文档的可读性与完整性。
- **MCP 本地配置安全治理**：将仓库内 `mcp.json` 调整为脱敏模板，新增本地私有配置约定（`mcp.local.json`，已加入忽略规则），避免将飞书应用密钥直接提交到版本库。
- **版本号更新**：将版本号从 `2.6.4` 更新为 `2.6.5`，同步更新 `package.json`、`server.json` 与 `README.md` 中的版本标识。
...
- **OAuth 回调端口冲突处理**：在 stdio 传输模式下，如果 OAuth 回调 HTTP 端口（默认 3010）已被占用，则跳过启动专用回调服务器，仅记录日志并假定已有实例在处理回调，避免多项目 / 多进程场景下因端口冲突导致 MCP 启动失败。
- **多实例兼容性**：HTTP 传输继续使用端口重试机制（从 3010 起自动递增），而 OAuth 回调服务器在端口被占用时自动让出职责，使多个项目可以同时加载并共享同一套 Feishu 授权状态。
- **版本号更新**：将版本号从 `2.6.2` 更新为 `2.6.3`，同步更新 `package.json` 与 `server.json` 中的版本字段。
